...
/**
 * Explore, phase one: the Universe is a searchable, filterable list grouped by
 * embodiment. The nested-territories treemap arrives from @ri/viz and drops into
 * the same slot without changing this page's data access.
 */
export default async function ExplorePage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return (
    <div className="flex max-w-[1180px] flex-col gap-5">
      ...
      {groups.length === 0 ? (
        <p className="py-10 text-[13px] text-ink-3">Nothing matches those filters.</p>
      ) : (
