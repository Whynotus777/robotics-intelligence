...
`HAS_ROLE` also supports `DATA_SIMULATION_PROVIDER` (robot training data and simulation) and `TOOLING_OPS_PROVIDER` (robot development, observability and fleet operations software). EMS organizations use `CONTRACT_MANUFACTURER`. Product categories are not roles. `HAS_TICKER` stores an exchange-qualified listing symbol; private organizations omit it. `FOUNDED_ON` allows year precision using January 1 with an explicit year-precision qualifier. `PART_OF` requires primary or credible third-party control evidence and a date. Use the effective closing date when documented. If only a current affiliation or reporting period is documented, date the claim from that confirmed point and explicitly qualify that the original acquisition date is unknown; never present an announcement awaiting approval as a completed acquisition. Group affiliations may be indirect and must say so.

The atlas includes `DATA_SIMULATION_PROVIDER` in TRAINED and `TOOLING_OPS_PROVIDER` in PLATFORMS. Organization headquarters provide discovery locations, not evidence of a deployment. Roster-only headquarters are ANALYST/LOW claims with a provisional qualifier, an assessment, and a source pinned to the original roster revision. Missing founding years or unverified listings are omitted; a missing ticker does not imply private ownership. Year-precision founding claims use `approximate: true`.
